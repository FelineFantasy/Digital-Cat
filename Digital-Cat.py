...
def random_event(cat: CatState):
    ...
    if random.random() < 0.15:
        ...
        if event == 1:
            ...
            gift_money = random.randint(5, 15)
            cat["money"] += gift_money
            print(f"{CYAN}Кот {cat['name']} принёс вам подарок!{RESET}")
            print(f"Он принёс {gift}! Вы продали это за {gift_money} монет.")
            log_to_file("DEBUG", f"Событие: кот принёс подарок ({gift}), монет +{gift_money}")
        elif event == 6:
            cat["happiness"] -= 10
            cat["love"] -= 5
...
def action_stats(cat: CatState):
    ...
    print(f"День: {cat['day']}")
    print(f"Монеты: {cat['money']}")
...
def get_os_greeting():
    ...
    if system == "Windows":
        return r"""🪟 Windows (ОС, которая перезагружается, когда ей вздумается)
(c) Корпорация "Окно в мир синего экрана". Все права защищены, но мы всё равно взломаем.
Версия: Вам точно нужно обновление. Или переустановка. Или новый компьютер (Или Linux)."""
    elif system == "Linux":
...
Версия: Вы явно переплатили за этот компьютер."""
        ...
        return r"""🤖 Android (ОС для карманных роботов)
(c) Корпорация "Зелёный робот". Все права защищены, но код открыт.
Версия: Вы держите в руках целый мир."""
    elif system == "iOS":
        return r"""🍏 iOS (ОС для тех, кто любит порядок)
(c) Корпорация "Надкусанное яблоко". Все права защищены до суда.
Версия: Вы явно платите за экосистему."""
    else:
